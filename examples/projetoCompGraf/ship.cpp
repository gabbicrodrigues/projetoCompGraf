...
void Ship::update(const GameData &gameData, float deltaTime) {
  ...
  if (gameData.m_input[static_cast<size_t>(Input::Left)] &&
      gameData.m_state == State::Playing) {
    // Thrust in the forward vector
    glm::vec2 esquerda = glm::rotate(glm::vec2{-0.5f, 0.0f}, m_rotation);
    m_velocity += esquerda * deltaTime;
  }
  ...
  if (gameData.m_input[static_cast<size_t>(Input::Right)] &&
      gameData.m_state == State::Playing) {
    // Thrust in the forward vector
    glm::vec2 direita = glm::rotate(glm::vec2{0.5f, 0.0f}, m_rotation);
    m_velocity += direita * deltaTime;
  }
}
